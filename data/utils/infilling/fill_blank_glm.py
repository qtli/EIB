...
import sys
# SCRIPT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'glm')  # 'XXX'
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))  # 'XXX/infilling'
sys.path.insert(1, os.path.dirname(SCRIPT_DIR))

from glm import mpu
from datetime import datetime
from glm.arguments import get_args
from glm.pretrain_glm import initialize_distributed
from glm.pretrain_glm import set_random_seed
from glm.pretrain_glm import get_masks_and_position_ids
from glm.configure_data import prepare_tokenizer
from glm.train_utils import get_model
from glm.utils import load_checkpoint
from glm.generation_utils import BeamSearchScorer
...
def process_one_context(tokenizer, args, output, raw_text):
    terminate_runs, skip_run = 0, 0
    if mpu.get_model_parallel_rank() == 0:
        generation_mask = '[gMASK]' if args.task_mask else '[MASK]'
        if args.block_lm and 'MASK]' not in raw_text:
...
def glm_generate_one_sample(model, tokenizer, args, device, raw_text):
    model.eval()
    output_path = "./samples"
    ...
    with torch.no_grad(), open(output_path, "w") as output:
        ...
        terminate_runs, raw_text, context_tokens_tensor, context_length = process_one_context(tokenizer, args, output, raw_text)
        if terminate_runs == 1:
            return
        ...
        if mpu.get_model_parallel_rank() == 0:
            ...
            decode_tokens = tokenizer.DecodeIds(output_tokens_list.tolist())
            trim_decode_tokens = decode_tokens
            # print("\nGLM:", trim_decode_tokens,
            #       flush=True)  # 使用flush=True之后，会在print结束之后，不管你有没有达到条件，立即将内存中的东西显示到屏幕上，清空缓存
            # output.write(trim_decode_tokens + "\n")

        torch.distributed.barrier(group=mpu.get_model_parallel_group())
        return trim_decode_tokens  # trim_decode_tokens.split('<|startofpiece|>')
    ...
    with torch.no_grad(), open(output_path, "w") as output:
        while True:
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                decode_tokens = tokenizer.DecodeIds(output_tokens_list.tolist())
                trim_decode_tokens = decode_tokens
                print("\nGLM:", trim_decode_tokens, flush=True)  # 使用flush=True之后，会在print结束之后，不管你有没有达到条件，立即将内存中的东西显示到屏幕上，清空缓存
                output.write(trim_decode_tokens + "\n")
...
def glm_setup():
    ...
    args.out_seq_length = 64  # 512
    args.seq_length = 64  # 512
    args.num_beams = 4
    ...
    cur_dir = os.getcwd()
    args.load_pretrained = os.path.join(cur_dir, "glm/blocklm-2b-512")

    args.mem_length = args.seq_length + args.mem_length - 1  # 512 + 0
...
if __name__ == '__main__':
    model, tokenizer, args, device = glm_setup()
