...
import torch
import pickle
import glob  #  文件操作相关模块，查找符合自己目的的文件
import time
import numpy as np
...
def get_all_sentences(unify_data, to_file):
    ...
    for data_name in unify_data:  # 遍历每个数据集
        ...
        for split in ['train', 'dev', 'test']:  # 遍历所有子集
            split_data = unify_data[data_name][split]['explanation_opt'][:50000]  # todo: 每个数据集最多处理50000
...
def gen_sentence_embeddings(all_sentences_file, args):
    ...
    all_sentences = src.data.load_sentences(all_sentences_file, data_name=None)
    total_sentences = 0
    ...
    if args.shard_id == args.num_shards - 1:  # true
        end_idx = total_sentences  # 遍历全部句子
...
def index_encoded_data(index, ids, embeddings, indexing_batch_size):
    '''
    主要作用是，把embeddings分成不大于indexing_batch_size的块，每个块输入给index。
    :param index:
    :param ids:
    :param embeddings:
    ...
    with torch.no_grad():
        ...
        for k, q in enumerate(queries):
            ...
            if args.normalize_text:  # false
                q = src.normalize_text.normalize(q)  # Lowercase and remove quotes from a TensorFlow string，比如 text = tf.strings.regex_replace(text,"'(.*)'", r"\1")
...
def add_passages(queries, sentence_id_map, top_sentences_and_scores):
    ...
    for i, d in enumerate(queries):
        ...
        ctxs_num = len(docs)
        merged_data[d] = []  # key是检索输入，value是相似句子列表
        for c in range(ctxs_num):
            if docs[c] != d:   # 去掉相似句子列表中的key本身
                merged_data[d].append(
                    {
                        "id": results_and_scores[0][c],  # 相似的邻居句子ids
                        "sentence": docs[c],
                        "score": scores[c],
                    }
                )
...
def sentence_retrieval(all_sentences_file, args):
    ...
    # index all passages
    input_paths = glob.glob(args.sentences_embeddings)  # 回所有匹配的文件路径列表
    input_paths = sorted(input_paths)
    embeddings_dir = os.path.dirname(input_paths[0])  # returns the directory name of the pathname path, e.g., /apdcephfs/share_916081/qintongli/Explanation_code/process_data/get_negative_x/contriever_embeddings
    ...
    for i, file_path in enumerate(input_paths):
        ...
        with open(file_path, "rb") as fin:  # 读取embeddings文件
            allids, allembeddings = pickle.load(fin)
            allmerged_data = {}

            for data_name in allids.keys():  #  为每个数据集单独建立索引
                print(f'data_name: {data_name}')
                if all_sentences[data_name] == []:
                    continue
                # 每个数据集建立一个index
                index = src.index.Indexer(args.projection_size, args.n_subquantizers, args.n_bits)  # projection_size=768, n_subquantizers=0, n_bits=8
                data_embeddings_dir = os.path.join(embeddings_dir, data_name)
                os.makedirs(data_embeddings_dir, exist_ok=True)
                index_path = os.path.join(data_embeddings_dir, "index.faiss")   # 每个数据集一个单独的index文件, index.faiss文件名不能换

                if args.save_or_load_index and os.path.exists(index_path):  # save_or_load_index=False
                    index.deserialize_from(data_embeddings_dir)  # 直接取用已有的文件
                else:
                    index_encoded_data(
                        index,
                        allids[data_name],
                ...
                sentence_id_map = {(idx+1): q for idx, q in enumerate(queries)}   # 这里的（idx+1）要与 embed_sentences函数里的 batch_ids.append(k+1) 对应
            ...
            output_path = os.path.join(args.output_dir, 'merged_'+os.path.basename(args.sentences))  # output_dir: contriever_nq 存放结果，os.path.basename 返回path最后的文件名
...
def parse_params():
    ...
    # unify_data_file = /home/qtli/explanation/Explanation_code/NLEXP-datasets/unify_expl_dataset.json
    parser.add_argument("--unify_data_file", type=str, default='/apdcephfs/share_916081/qintongli/Explanation_code/NLEXP-datasets/unify_expl_dataset0810.json', help="Path to unify_data_file (.json file)")
