...
# def get_scores_new(inputs, model_type, model, tokenizer, data_collator, device=None, batch_size=32, verbose=False):
# def get_scores(inputs, model_type, device=None, batch_size=32, verbose=False):
#     '''
#     Inputs:
#       - a list of explanations to score, e.g.,:
...
def setup_before_test():
    ...
    args.model_type = 'large'
    args.batch_size = 1
    args.device = 'cuda'

    model = get_model(args.model_type, device=args.device)
    ...
    return args, model, tokenizer, data_collator
