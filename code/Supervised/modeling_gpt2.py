...
# from torch_scatter import scatter_max, scatter_mean, scatter_add
sys.path.append('/apdcephfs/share_916081/qintongli/Explanation_code/transformers')
...
class TextGen(GPT2PreTrainedModel):
    ...
    def aotoreg_generate(
        ...
        output_sequences = self.generate(
            input_ids=input_ids,
            # attention_mask=attention_mask,
            max_length=input_lens + args.target_length,
            temperature=args.temperature,
            top_k=args.sampling_topk,
            top_p=args.sampling_topp,
            do_sample=args.sampling,
            num_return_sequences=args.beam
        )
        ...
        for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
            ...
            total_sequence = text[len(self.tokenizer.decode(input_ids[0][: ], clean_up_tokenization_spaces=True)):].strip(' ').replace('\n', '<n>')  # .masked_select(input_ids[0] != self.tokenizer.pad_token_id)
